/** 
 * I have do some extensions to my Observables which is 
 * delay and takeWhile
 * 
 * delay: 
 * The reason I added it is because at first I thought I could use it to
 * do some delay subscription after my counting down completed,
 * but then I realised I could actually achieve it without using this
 * method but with a shorter and easier way, which is subcribe the elems which I 
 * would like to delay inside the countdown's observer's complete function.
 * 
 *
 * takeWhile:
 * Looks like a combination of filter and takeUntil,
 * which will emit the value from the source observable until the emitted 
 * value is unsatisfied to the predicate(function).
 * I used this method to unsubcribe all my Elems in the oberver's complete function 
 * when there exists someone who has scored to 11. 
 */
...
class Observable<Input> {
   ...
   * @return Obsevable that delays the emissions of the source Observable by the specified timeout.
   */
  delay(delay: number) : Observable<Input> {
    return new Observable<Input>((observer: Observer<Input>) =>
      this.subscribe(e=>{
        setTimeout(()=>observer.next(e), delay);
      },
      ()=>setTimeout(()=>observer.complete(), delay)));
  }

  /**
   * Emits value by the source Observable until which value is not satisfied the predicate, then it completes.
   * https://rxjs-dev.firebaseapp.com/api/operators/takeWhile
   * @param condition a function that evaluates a value emitted by the source Observable and returns a boolean.
   * @return An Observable that emits the values which satisfied the predicate from the source Observable and completes after it unsatisfied.
   */
